...
fun main() {
    ...
    val parsingJson = Json {
        ignoreUnknownKeys = true
    }

    fun parseBundleMetadata(variant: BundleVariant): ParsedBundle? {
        val content = variant.file.readText()
        return try {
            val bundle = parsingJson.decodeFromString<BundleFile>(content)
            val version = normalizeMetadataValue(bundle.version)
            if (version == null) {
                Logger.warning("Version is invalid.")
                return null
            }
            val downloadUrl = normalizeMetadataValue(bundle.downloadUrl)
            if (downloadUrl == null) {
                Logger.warning("Download URL is invalid.")
                return null
            }
            ParsedBundle(version, downloadUrl)
        } catch (_: SerializationException) {
            try {
                val legacyBundle = parsingJson.decodeFromString<LegacyBundleFile>(content)
                val patches = legacyBundle.patches
    ...
    fun processBundle(bundleFolder: File) {
        ...
        val variants = bundleFolder
            ...
            ?.filter { it.name.endsWith("-patches-bundle.json") }
            ?.map { file ->
            ...
            .forEachGroupLogged(
                ...
                try {
                    ...
                    if (outputPatchesFile.exists()) {
                        existingContent = parsingJson.decodeFromString<LocalPatchesFile>(
                            outputPatchesFile.readText()
                        ).also {
                            if (it.version != parsedBundle.version) {
                                Logger.info("Version ${it.version} -> ${parsedBundle.version}")
                            } else {
                    ...
                    outputPatchesFile.writeText(
                        prettyJson.encodeToString(
                            LocalPatchesFile(parsedBundle.version, generated)
                        )
                    )
                } catch (e: Exception) {
